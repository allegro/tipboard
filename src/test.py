...
def testTileUpdate(tester=None, tileId='test_pie_chart', sonde=None):
    """
    ...
    tile_txt_data_before = json.loads(getCache().redis.get(tilePrefix))
    sonde(tester=tester, tile_id=tileId)
    tile_txt_data_after = json.loads(getCache().redis.get(tilePrefix))
    isDiff = tile_txt_data_before['data']['datasets'][0]['data'] != tile_txt_data_after['data']['datasets'][0]['data']
